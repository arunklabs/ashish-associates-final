...
const CounterAnimation = ({
  ...
  useEffect(() => {
    // Don't animate if no number is provided
  ...
  return (
    ...
    >
      <div className="relative">
        {end !== undefined && (
          <div
            className={`text-4xl md:text-5xl font-heading font-bold mb-2 group-hover:scale-110 transition-transform duration-300 ${theme === "dark" ? "text-primary" : "text-[#C9A646]"
              }`}
          >
            {count}
            {suffix}
      ...
      <div
        className={`text-sm uppercase tracking-widest mt-4 ${theme === "dark" ? "text-muted-foreground" : "text-gray-600"
          }`}
      >
        {label}
      </div>
